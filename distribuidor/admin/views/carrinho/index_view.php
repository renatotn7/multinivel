                ...
                foreach ($carrinho_compra as $carrinho) {
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <i style="display: table; cursor: pointer;" onclick="add(<?php echo $carrinho->pm_id; ?>);" class="fa fa-arrow-up"></i>
                            <i style="display: table; cursor: pointer;" onclick="retira(<?php echo $carrinho->pm_id; ?>);" class="fa fa-arrow-down"></i>
                        </td>
                        <td>
                            <strong>US$:</strong><?php echo number_format(($carrinho->pr_valor * $carrinho->pm_quantidade), 2); ?>
                            <?php echo $carrinho->co_pago == 1 ? '' : '<a href="' . base_url("index.php/loja/excluir_produto_carrinho?id_pedido={$carrinho->co_id}&id_prod={$carrinho->pr_id}") . '"><i class="fa fa-remove"></i></a>'; ?>
                            <?php // echo $carrinho->co_pago == 1 ? '' : '<a href="' . base_url("index.php/loja/excluir_produto_carrinho?id_pedido={$carrinho->co_id}") . '">< i class="fa fa-remove"></i>' . $this->lang->line('label_remover') . '</a>'; ?>
                        </td>
                    </tr>
                    <?php
                }
