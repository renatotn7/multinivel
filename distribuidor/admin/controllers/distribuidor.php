...
class Distribuidor extends CI_Controller {
    ...
    public function verificar_token_pagamento_ajax() {
        try {
            ...
            $agencia = PlanosModel::getPlanoDistribuidor(get_user()->di_id);

            if ($dados->prk_agencia != $agencia->pa_id) {
                throw new Exception('Error');
            }
    ...
    public function verificar_conta() {
        ...
        if (!DistribuidorDAO::contaverificada(get_user())) {
            redirect(base_url());
        }
        ...
        if ($this->input->post('plano')) {
            ...
            if (count($co_plan) == 0) {
                ...
                $this->db->insert('compras', array(
                    ...
                    'co_total_valor' => $plano_upgrade->pr_valor,
                ...
                $co_descricao = "Upgrade para do plano {$planoAtual->pa_descricao} para o plano {$plano->pa_descricao}";
                //gerando uma nova comprar do plano do distribuidor.
                $this->db->where('co_id', $co_plan->co_id)->update('compras', array(
                    'co_total_pontos' => $plano_upgrade->pug_pontos,
                    'co_id_plano' => $plano->pa_id,
                    'co_descricao' => $co_descricao,
                    'co_total_valor' => $plano_upgrade->pr_valor,
                ...
                //Pega o id do plano e atualizar
                $this->db->where('pm_id_compra', $co_plan->co_id)->update('produtos_comprados', array(
                    'pm_valor' => $plano_upgrade->pr_valor,
                    'pm_pontos' => $plano_upgrade->pug_pontos,
                    'pm_id_produto' => $plano_upgrade->pug_produto,
                    'pm_valor_total' => $plano_upgrade->pr_valor,
...
//            }
        ...
        $this->db->where('di_id', get_user()->di_id)->update('distribuidores', valida_fields('distribuidores', $_POST));
        $end = $this->db->where('end_id_distribuidor', get_user()->di_id)->get('distribuidores_endereco')->row();
        if (count($end) > 0) {
            $this->db->where('end_id_distribuidor', get_user()->di_id)->update('distribuidores_endereco', valida_fields('distribuidores_endereco', $_POST));
        } else {
            $_POST['end_id_distribuidor'] = get_user()->di_id;
            $this->db->insert('distribuidores_endereco', valida_fields('distribuidores_endereco', $_POST));
        }
...
//End função validação de dados
    ...
    public function buscar_distribuidor_ajax() {
        ...
        $valor = $_POST['chave'] + 0;
        ($_POST['chave'] === '' . $valor) ? $this->db->where('di_id', $_POST['chave']) : $this->db->like('di_usuario', $_POST['chave']);
    ...
    public function enviar_token_produto() {
        ...
        try {
            ...
            //saldo que usuário pode sacar EWC %85
            $atm = new atm();
            $retorno = $atm->solicitar_saque($dados, ($dados->prk_reembolso * 0.85));
            ...
            $response = funcoesdb::arrayToObject($retorno);
            ...
            //Valor do Ewc- voucher.
            $response = funcoesdb::arrayToObject($atm->solicitar_saque($dados, ( $dados->prk_reembolso * 0.15), 1));
            switch ($response->status) {
                case 2:
                    throw new Exception($this->lang->line('error_usuario_nao_existe'));
                    break;
            ...
            ComprasModel::utilizarTokenProdutoComprado($this->input->post('token'), get_user());
    ...
    public function reativar_distribuidor() {
        ...
        try {
            ...
            if ($this->input->post('paymentMethod') === false) {
                throw new Exception($this->lang->line('error_selecione_opcao_pagamento'));
            }
            ...
            //Verificando se o distirbuidor já não tem essa compra constando como paga no sistema.
            $compra_ativacao = produtoModel::getProdutoComprados(get_user(), 8);
            ...
            redirect(base_url('index.php/pedidos/confirmar_pagamento?id_pedido=' . $compra_ativacao->co_id) . '&paymentMethod=' . $this->input->post('paymentMethod'));
