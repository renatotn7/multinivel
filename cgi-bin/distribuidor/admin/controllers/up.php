...
class Distribuidor extends CI_Controller {
    ...
    public function teste() {
        $atm = new atm();
        var_dump($atm->consulta_cadastro_ewallet($_REQUEST["email"]));
    }
...
// 	  $emialAtual = $this->db->where('di_email',$_POST['di_email'])->get('distribuidores')->row();
// 	if(count($emialAtual) == 0)  
//        self::validar_senha_seguranca($_POST['senha_segurancao'],$_POST['url']);
        //Valida��o dos dados
        ...
        //pegando informa��es  para salvar informa��es do distribuidor
