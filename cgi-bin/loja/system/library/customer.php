...
class Customer {
  	...
  	public function __construct($registry) {
		...
		if (isset($this->session->data['customer_id'])) { 
			...
			if ($customer_query->num_rows) {
				...
				$this->consultor_id = $customer_query->row['consultor_id'];
	...
	public function logout() {
  	...
  	public function getRewardPoints() {
        ...
        public function getConsultorId() {
		return $this->consultor_id;	
  	}
        

        public function getLoginConsultor(){
            $query = $this->db->query("SELECT di_usuario FROM distribuidores WHERE di_id = '" . (int)$this->consultor_id . "'");
            if($query->num_rows){
                return $query->row['di_usuario'];
            }
        }
        
        public function getIDConsultor($usuario=''){
            $query = $this->db->query("SELECT di_id FROM distribuidores WHERE di_usuario = '" . $usuario . "'");
            if($query->num_rows){
                return $query->row['di_id'];
            }else{
                return false;
            }
        }
        
        public function eConsultor(){
            return $this->getConsultorId()>0;
        }
}
?>