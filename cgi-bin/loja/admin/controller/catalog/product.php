...
class ControllerCatalogProduct extends Controller {
    ...
    public function stock() {
        ...
        if ($this->user->getStoreId() != 0) {
            $this->redirect($this->url->link('catalog/product/', 'token=' . $token));
        }

        $this->template = 'catalog/product_stock.tpl';
        ...
        $this->data['movimentacoes'] = $this->model_catalog_product->getHandlingStock($idProduct);
        $this->data['product'] = $this->model_catalog_product->getProduct($idProduct);
        $this->document->setTitle($this->data['product']['name']);
        $this->data['token'] = $this->request->get['token'];
        $this->data['product_id'] = $idProduct;
    ...
    public function add_stock() {
        ...
        $idProduct = $this->request->get['product_id'];
        $product = $this->model_catalog_product->getProduct($idProduct);
        if (count($product) == 0) {
            $this->redirect($this->url->link('catalog/product/', 'token=' . $token));
            exit;
        }

        if ($this->request->post['qtd'] == 0) {
            $this->redirect($this->url->link('catalog/product/stock', 'token=' . $token . '&product_id=' . $idProduct));
            exit;
        }

        if ($this->request->post['type'] == 1) {
            $this->model_catalog_product->addStock($idProduct, $this->request->post['qtd'], 0);
        } elseif ($this->request->post['type'] == 2) {
            $this->model_catalog_product->addStock($idProduct, 0, $this->request->post['qtd']);
        }
    ...
    public function delete() {
        ...
        if (isset($this->request->post['selected']) && $this->validateDelete()) {
            ...
            foreach ($this->request->post['selected'] as $product_id) {
                 ...
                 if($product['remover'] == 1){
                   $this->model_catalog_product->deleteProduct($product_id);
                 }else{
                     $this->error['warning'] = 'Você não tem permissão para remover o produto '.$product['name'];
                 }
            }
    ...
    private function getList() {
        ...
        //werlon
        $this->data['refresh_special_product'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . $url, 'SSL');
        ...
        //werlon
        $this->data['button_special_product'] = $this->language->get('button_special_product');
    ...
    //
    public function gera_desconto() {
        $this->load->model('catalog/product');
        //$json = array();
        
        if (isset($this->request->get['desconto']) && $this->request->get['desconto']!='') {
            $result = $this->model_catalog_product->aplicarDescontoFranquiados($this->request->get['desconto']);
        }else{
            $result = $this->model_catalog_product->aplicarDescontoFranquiados();
        }
        //$result = $this->model_catalog_product->aplicarDescontoFranquiados($this->request->get['desconto']);;
        //$result = $this->model_catalog_product->aplicarDescontoFranquiados();
        $json['success'] = $result;
