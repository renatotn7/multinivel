...
if (!function_exists('redirectLastTime')) {
/**
 * Redireciona com um retado de tempo
 * @param type $uri
 * @param type $time
 * @param type $method
 * @param type $http_response_code
 */
    function redirectLastTime($uri = '', $time = 1, $method = 'location', $http_response_code = 302) {
        if (!preg_match('#^https?://#i', $uri)) {
            $uri = site_url($uri);
        }
        $time = $time * 1000;
        switch ($method) {
			...
			<head>
			<script type='text/javascript'>
                          setTimeout(\"document.location = '{$uri}'\",{$time});
			</script>
			</head>
			</html>";
                exit;
