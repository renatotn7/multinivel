...
class bonusPL {
     ...
     * @return type
     */
    public static function getDistribuidorInstance($idDistribuidorOrUsuario = 0) {
        ...
        if (!isset(self::$distribuidor_instance)) {
            ...
            self::$distribuidor_instance = get_instance()->db->where('di_excluido', 0)
                            ->where('co_pago', 1)
    ...
    public function run($distribuidor = array()) {
        ...
        foreach (self::getDistribuidorInstance($di_id)as $distribuidor) {
            ...
            try {
                ...
                echo "usuario: $distribuidor->di_usuario - Data: " . $this->data_pl() . "\n";
                ...
                if (count($plano) == 0) {
                    echo ("Usuário não conseguir se qualificar em nenhum plano até o momento.");
                }
    ...
    public function regra_nova($distribuidor, $plano) {
        ...
        //Descricao da PL.
        $descricao_pl = 'Bônus PL <b>' . date('d/m/Y', strtotime($this->data_pl())) . '</b>';

        //Montar a descrição do bônus
        if (number_format($valor_pl, 2, '.', '') != $valor_bonus_pl) {
            $descricao_pl = 'Bônus PL <b>' .
    ...
    public function regra_antiga($distribuidor) {
        ...
        if ($this->getConfig('valor_pl_hoje') != $valor_bonus_pl) {
            $descricao_pl = 'Bônus PL <b>' . date('d/m/Y', strtotime($this->data_pl())) . '</b><br>
			   Você atingiu o limite de <b>US$ ' .
