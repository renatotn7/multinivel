...
class Binario {
    ...
    private function verificar_binario_ativo() {
        if ($this->distribuidor->di_binario == 1) {
            $this->binario_ativo = true;
            return;
        }
       
        if ($this->get_indicacoes_diretas_direita() > 0 && $this->get_indicacoes_diretas_esquerda() > 0) {
            $AT = new AtivacaoBinario($this->distribuidor);
            $AT->ativarBinarioDistribuidor();
            $this->binario_ativo = true;
            return;
        }
